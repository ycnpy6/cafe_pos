...
public class ProductDAO {
    public List<Product> findActiveByCategory(int categoryId) throws Exception {
        String sql = "SELECT id, name, price, cost, category_id, stock, active, is_prepared, linked_ingredient_id " +
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    results.add(new Product(
                            ...
                            (Integer) rs.getObject("linked_ingredient_id")
                    ));
                }
            }
        }
    ...
    public List<Product> findAll() throws Exception {
        String sql = "SELECT id, name, price, cost, category_id, stock, active, is_prepared, linked_ingredient_id FROM products ORDER BY name";
        List<Product> results = new ArrayList<>();
        try (Connection conn = DatabaseManager.openConnection();
             PreparedStatement ps = conn.prepareStatement(sql);
    ...
    public int insertProduct(Product product) throws Exception {
        String sql = "INSERT INTO products (name, price, cost, category_id, stock, active, is_prepared, linked_ingredient_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = DatabaseManager.openConnection();
            ...
            ps.setObject(8, product.getLinkedIngredientId());
            ps.executeUpdate();
    ...
    public Product findById(Connection conn, int productId) throws Exception {
        String sql = "SELECT id, name, price, cost, category_id, stock, active, is_prepared, linked_ingredient_id FROM products WHERE id = ?";
    ...
    public void updateLinkedIngredient(int productId, Integer ingredientId) throws Exception {
        String sql = "UPDATE products SET linked_ingredient_id = ? WHERE id = ?";
        try (Connection conn = DatabaseManager.openConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
